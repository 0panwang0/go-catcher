...
const SERVER_BASE = "http://127.0.0.1:7891";

async function pingServer(timeoutMs = 2000) {
  try {
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), timeoutMs);
    const resp = await fetch(`${SERVER_BASE}/health`, { signal: ctrl.signal });
    clearTimeout(timer);
...
async function downloadViaServer({ m3u8Url, referer, title, filename } = {}) {
  ...
  try {
    ...
    const pkRes = await fetch(`${SERVER_BASE}/pickdir`, {
      method: "GET",
      cache: "no-store",
    });
    if (!pkRes.ok) {
    ...
    const safeFn = filename && filename !== "video.mp4" ? filename : `${title || "video"}.mp4`;
    params.set("filename", safeFn);

    const dlRes = await fetch(`${SERVER_BASE}/download?${params.toString()}`, {
      method: "GET",
      cache: "no-store",
    });
    if (!dlRes.ok) {
...
async function queryDownload(msg) {
  ...
  try {
    ...
    if (taskId) {
      // 多任务：按任务 id 精确查
      const resp = await fetch(`${SERVER_BASE}/status?id=${encodeURIComponent(taskId)}`, { cache: "no-store" });
      ...
      const resp = await fetch(`${SERVER_BASE}/status`, { cache: "no-store" });
...
async function controlTask({ taskId, action } = {}) {
  ...
  try {
    const resp = await fetch(
      `${SERVER_BASE}/${path}?id=${encodeURIComponent(taskId)}`,
      { cache: "no-store" }
    );
