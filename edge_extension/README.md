...
把之前的 Go 版 m3u8 下载器做成了 Edge（Chromium）扩展。扩展的请求走浏览器自身的网络栈，TLS 指纹天然真实，**不再需要额外的指纹伪装**；代理直接跟随浏览器的系统代理设置（开启系统代理即可生效）。
...
2. 客户端打开后会**自动启动下载服务**，并把自己登记为浏览器的**原生消息宿主**
3. 之后所有视频下载都自动通过这个服务完成，无需再手动开终端

> **服务没跑也能点。** 客户端运行过一次之后，扩展检测到服务不在运行时会通过原生消息
> 通道自动把客户端唤起来（**托盘常驻形态，不弹主窗口**），再继续下载——不必再惦记
> "先打开客户端"这一步。
> 只要托盘不要界面：终端里运行 `go-catcher.exe --tray`（就是被唤起时的那个形态）。
> 只要服务不要界面：终端里运行 `go-catcher.exe --server`（无头模式）。
> 下载请求按系统代理设置出网；要指定固定代理地址，在 GoCatcher 设置页里填。

唤起失败时（宿主未登记、或被浏览器策略拦下），扩展会提示"请先打开 go-catcher.exe"，
并给出"复制命令到终端"的兜底路径。查看登记状态：`go-catcher.exe --native-host-status`。
...
3. 点击后按链接数量分流。列表**只包含所悬停视频的链接**，逐级过滤（每级剔光即回退，宁多勿漏）：
   ...
   - **同一视频页**：同域名同路径，排除此前浏览的其他视频页残留
   - **正在播放的流**（三级，每级剔光即回退）：先按**最新一条媒体请求的目录**严格匹配——正在播放的流必然刚拉过 .ts/.m3u8/.flv，旧流切走后残留的余波请求目录更旧被挤掉；不中再按最近 15 秒全部活跃目录宽松匹配；暂停播放无信号时回退全列表

   链接的画质/时长信息通过**本地 Go 服务的 `/probe` 接口**预检（服务端带 Referer + 浏览器指纹直连——浏览器对无 CORS 头的 CDN 只能拿到空响应，这是预检必须走服务端的原因）；GoCatcher 未运行时预检降级为页面 fetch，拿不到信息的条目标"未预检"灰显排在列表末尾。列表按视频分组聚拢，每行显示格式/画质/分辨率/码率/时长/估算大小/URL：
   - **只有 1 个链接**：直接弹出**确认面板**
...
HLS 分片按序拼接的原始流直接保存，**不做任何封装转换、不依赖任何外部转封装工具**。
输出扩展名由**实际探测到的容器**决定（不是由链接名或调用方指定）：

- **TS**（`.ts`，MPEG-TS）：主流播放器靠内容嗅探即可正常播放；需要标准 MP4 容器时，用任意转封装工具做一次流复制转换（秒级完成，不重新编码）
...
- TLS 指纹伪装（解决 JA3 检测）
- 走系统代理设置（可在设置页改为手动指定 / 直连）
- 10 并发 + 3 次重试 + 按序落盘（无独立合并阶段，`next` 即断点，支持暂停/崩溃续传）
- 已端到端验证可下载 2.7GB 1080p 视频（约 1 分钟）

## 代理说明（重要）

扩展的网络请求走 **Edge 浏览器自己的代理设置**，无法在代码里强制指定代理地址。
因此需要系统层面的代理生效，任选其一：

- 开启 **系统代理** 模式（浏览器默认跟随系统代理）
- 或开启 **TUN / 虚拟网卡** 模式（接管全部流量）
- 或在浏览器设置里手动配置代理服务器地址

如果代理工具只开了本地端口、没设系统代理，扩展请求不会走代理，会 403 或超时。

## 403 说明

旧版本在扩展页（`chrome-extension://...`）直接 fetch，Origin 头会被 CDN 识别为非页面请求而 403。

当前版本：**所有实际下载请求都由本地 Go 服务发起**，服务端带 Referer + 浏览器指纹直连，与视频正常播放时的请求特征一致，403 问题已从根本上解决。若仍遇到 403，请检查系统代理是否已开启（或是否处于 TUN / 虚拟网卡模式）。
...
| Go 版 | 扩展版 |
|---|---|
| TLS 指纹伪装 | 浏览器原生指纹，无需伪造 |
| 手动指定代理地址（或跟随系统代理） | 跟随浏览器系统代理 |
...
├── content.js         # 页面内悬停下载按钮 + 链接列表面板 + 交给本地服务下载
...
> 目录内**不能有以 `_` 开头的文件/目录**（浏览器保留给系统使用，会导致「无法加载扩展」）。
