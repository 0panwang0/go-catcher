...
func TestEffectiveProxySystemFollows(t *testing.T) {
	oldFn, oldProxy := systemProxyAddr, getProxyAddr()
	t.Cleanup(func() {
		systemProxyAddr = oldFn
	...
	// 系统代理已启用：透传注册表读数
	systemProxyAddr = func() string { return "http://10.1.1.1:9999" }
	if got := effectiveProxy(); got != "http://10.1.1.1:9999" {
	...
	systemProxyAddr = func() string { return "" }
	if got := effectiveProxy(); got != "" {
	...
	systemProxyAddr = func() string { t.Fatal("手动模式不应读系统代理"); return "" }
	setProxyAddr("http://manual:1")
	if got := effectiveProxy(); got != "http://manual:1" {
